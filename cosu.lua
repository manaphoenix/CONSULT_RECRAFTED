...
local tContent = {}
local tCursor = { ['x'] = 1, ['y'] = 1, ["lastX"] = 1, ["lastY"] = 1, ["autoListY"] = 0, ["selectedItem"] = 1,
    ["selectedSubDropdown"] = {} }
...
local blits = { [1] = '0', [2] = '1', [4] = '2', [8] = '3', [16] = '4', [32] = '5', [64] = '6', [128] = '7', [256] = '8',
    [512] = '9', [1024] = 'a', [2048] = 'b', [4096] = 'c', [8192] = 'd', [16384] = 'e', [32768] = 'f' }
local blitInvert = { ['0'] = 1, ['1'] = 2, ['2'] = 4, ['3'] = 8, ['4'] = 16, ['5'] = 32, ['7'] = 128, ['8'] = 256,
    ['9'] = 512, ['a'] = 1024, ['b'] = 2048, ['c'] = 4096, ['d'] = 8192, ['e'] = 16384, ['f'] = 32768 }
    ...
    elseif event == "now" then
        ...
        for index, pop in pairs(tPopup) do
            if pop.name == "Update CONSULT" then
                ...
                tPopup[index].button = {
                    { ['x'] = 42, ['y'] = 9, ["label"] = "Thanks", ["status"] = false,
                        ["func"] = function() update("close") end }
                }
                tPopup[index].text = {
                    {
                        "UPDATE COMPLETE",
    ...
    elseif event == "create" then
        ...
        if bAvailable then
            ...
            openPopup({
                ...
                ["button"] = {
                    { ['x'] = 42, ['y'] = 6 + #tChangelog, ["label"] = "Update", ["status"] = false,
                        ["func"] = function() update("now") end },
                    { ['x'] = 39, ['y'] = 6 + #tChangelog, ["label"] = "No", ["status"] = false,
                        ["func"] = function() update("close") end }
                }
            })
        end
...
function openLink(event, ...)
    ...
    elseif event == "create" then
        ...
        if fs.isDir(link) then return end
        if link == "" or link == "..." or link == "." or not fs.exists(link) or link:find('ö') or link:find('ä') or
            link:find('ü') then return end
        ...
        openPopup({
            ...
            ["button"] = {
                ...
                { ['x'] = #line - 6, ['y'] = 4, ["label"] = "No", ["status"] = false,
                    ["func"] = function() openLink("close") end }
            }
        })
    end
end
...
function info(event, ...)
    ...
    elseif event == "create" then
        openPopup({
            ...
            ["button"] = {
                { ['x'] = 31, ['y'] = 8, ["label"] = "Thanks", ["status"] = false,
                    ["func"] = function() info("close") end }
            }
        })
    end
end
...
function help(event, ...)
    ...
    elseif event == "create" then
        ...
        local popup = {
            ...
            ["text"] = {},
            ["button"] = {
                { ['x'] = 10, ['y'] = 10, ["label"] = "Next", ["status"] = false,
                    ["func"] = function(self) help("change", 1, self.page, #self.data) end },
                { ['x'] = 39, ['y'] = 10, ["label"] = "Done", ["status"] = false, ["func"] = function() help("close") end },
                { ['x'] = 1, ['y'] = 10, ["label"] = "Previous", ["status"] = false,
                    ["func"] = function(self) help("change", -1, self.page, #self.data) end }
            }
        }
...
function file(event, ...)
    ...
    elseif event == "replace" then
        openPopup({
            ...
            ["tmp"] = tArgs[1],
            ["button"] = {
                { ['x'] = 26, ['y'] = 3, ["label"] = "No", ["status"] = false,
                    ["func"] = function() file("close") file("create", "save as") end },
                { ['x'] = 29, ['y'] = 3, ["label"] = "Yes", ["status"] = false,
                    ["func"] = function(self) local tmp = self.tmp file("close") file("create", "save", tmp, "force") end }
            }
        })
    elseif event == "saved" then
        ...
        local pop = {
            ...
            ["button"] = {
                { ['x'] = nLength - 1, ['y'] = 4, ["label"] = "Ok", ["status"] = false,
                    ["func"] = function() file("close") end },
                { ['x'] = nLength - 6, ['y'] = 4, ["label"] = "Exit", ["status"] = false,
                    ["func"] = function() file("close") exit("create") end }
            }
        }
        table.insert(tPopup, 1, pop)
    ...
    elseif event == "create" then
        ...
        elseif tArgs[1] == "save as" then
            ...
            table.insert(tPopup, 1, {
                ...
                ["button"] = {
                    { ['x'] = 6, ['y'] = 4 + #tMsg, ["label"] = "Abort", ["status"] = false,
                        ["func"] = function() file("close") end },
                    { ['x'] = 12, ['y'] = 4 + #tMsg, ["label"] = "Done", ["status"] = false,
                        ["func"] = function() local tmp = tPopup[1].textBox[1].input
                            file("close")
                            file("create", "save"
                                , tmp)
                        end }
        ...
        elseif tArgs[1] == "new" then
            if multishell then
                ...
                openPopup({
                    ...
                    ["button"] = {
                        { ['x'] = 25, ['y'] = 4, ["label"] = "No", ["status"] = false,
                            ["func"] = function() file("close") end },
                        { ['x'] = 28, ['y'] = 4, ["label"] = "Yes", ["status"] = false,
                            ["func"] = function() file("close") file("create", "new", "force") end }
                    }
                })
            end
        end
...
function error(event, name, ...)
    ...
    elseif event == "create" then
        ...
        openPopup({
            ...
            ["text"] = tArgs,
            ["button"] = {
                { ['x'] = nLongestMsg - 1, ['y'] = nXCounter + 1, ["label"] = "Ok", ["status"] = false,
                    ["func"] = function() error("close") end }
            }
        })
    end
end
                    ...
                    for sConfigName, value in pairs(cosuConf) do
                        ...
                        elseif type(newValue) == "function" then
                            if type(newValue()) == type(value) then
                                ...
                                tWrongValues[#tWrongValues + 1] = "\'" ..
                                    sConfigName .. "\' expected \'" .. type(value) .. '\''
                            end
                        elseif type(newValue) == "nil" then
                            bMissingValues = true
                        else
                            tWrongValues[#tWrongValues + 1] = "\'" .. sConfigName .. "\' expected \'" ..
                                type(value) .. '\''
                        end
                    end
                end
    ...
    elseif event == "create" then
        ...
        if multishell then
            ...
            openPopup({
                ...
                ["button"] = {
                    { ['x'] = 15, ['y'] = 4, ["label"] = "Back", ["status"] = false,
                        ["func"] = function() options("close") end },
                    { ['x'] = 20, ['y'] = 4, ["label"] = "Edit", ["status"] = false,
                        ["func"] = function() options("close") options("create", "force") end }
                }
            })
        end
    end
    ...
    elseif event == "create" then
        ...
        openPopup({
            ...
            ["button"] = {
                ...
                { ['x'] = 24, ['y'] = 4, ["label"] = "Exit", ["status"] = false,
                    ["func"] = function() exit("JUST DO IT") end }
            }
        })
    end
end
...
function specialChar(event, ...)
    ...
    elseif event == "create" then
        local tNewPopup = {
            ...
            ["key"] = function(self, id, _)
                ...
                if self.list[1].type == "sidebyside" then
                    if id == cosuConf.tKeyboard.up and (self.list[1].selected - self.list[1].size.x - 1) > 0 then
                        ...
                        worked = true
                    elseif id == cosuConf.tKeyboard.down and
                        (self.list[1].selected + (self.list[1].size.x + 1)) < self.list[1].size.max then
                        self.list[1].selected = self.list[1].selected + (self.list[1].size.x + 1)
                        worked = true
                    end
                end
            ...
            ["button"] = {
                { ['x'] = 1, ['y'] = 1, ["label"] = "", ["status"] = false,
                    ["func"] = function(self) self:closeAtNoFokus() specialChar("close") end },
            }
        }
        local nSelected = 0
        for type, widget in pairs(tWidgets) do
        ...
        for _, i in pairs({ 198, 208, 216, 222, 230, 247, 254 }) do
            if nSelected == i then
                tNewPopup.list[1].selected = #tNewPopup.list[1].content + 1
            end
            tNewPopup.list[1].content[#tNewPopup.list[1].content + 1] = string.char(i)
...
function openShell()
    if shell.openTab then
        ...
        category.reset()
    else error("create", "multishell",
            { "The Multishell API is required,", "to run a shell in a new tab!", "(only available for advanced-PCs!)" })
    end
end
...
function draw.dropdownBG(nW, nH, nX, nY, tLineBreaks)
    ...
    if type(nY) == "nil" then nPopY = (h / 2) - (nH + 2) / 2 + 1
    end
    --[[ Draw main ]]
...
local tInfobar = {
    { --[[ Left side ]]
        function()
            ...
            return cToStr(colorMatch.special, colorMatch.bg) ..
                " " .. sShowPath .. cToStr(colorMatch.bg, cosuConf.cAccentColor) .. '\151'
        end
    }, { --[[ Right side ]]
        function() return cToStr(colorMatch.cAccentText) .. "Col " .. cToStr(colorMatch.text) .. tCursor.x .. " " end,
        function() return cToStr(colorMatch.cAccentText) ..
                "Ln " .. cToStr(colorMatch.text) .. tCursor.y .. cToStr(colorMatch.cAccentText) .. ","
        end,
        function()
            --[[ Calculate size ]]
            ...
            return cToStr(colorMatch.special) ..
                "[" ..
                cToStr(colorMatch.text) ..
                size .. cToStr(colorMatch.cAccentText) .. sizeType .. cToStr(colorMatch.special) .. "]"
        end,
    }
}
...
function input.insert.cursorHorizontal(sWay, bJump)
    ...
    elseif sWay == "right" then
        if tCursor.x <= #tContent[tCursor.y] then
            ...
            until not bJump or tCursor.x >= #tContent[tCursor.y] or
                tContent[tCursor.y]:sub(tCursor.x - 1, tCursor.x - 1):match('[ %(%)%.]')
...
function input.insert.cursorBackspace()
    if not (tCursor.y == 1 and tCursor.x == 1) then
        ...
        if sLine:sub(tCursor.x - cosuConf.nTabSpace, tCursor.x - 1) == (' '):rep(cosuConf.nTabSpace) then
            tContent[tCursor.y] = string.sub(sLine, 1, tCursor.x - cosuConf.nTabSpace - 1) ..
                string.sub(sLine, tCursor.x)
...
function input.menu.mouseClick(nButton, nX, nY)
    if nButton == 1 then
        ...
        if type(tPopup[1].button) == "table" then
            for nIndex, tB in pairs(tPopup[1].button) do
                if nX >= tPopup[1].x + tB.x and nX < tPopup[1].x + tB.x + #tB.label and nY == tPopup[1].y + tB.y then
                    if (cosuConf.bDoubleClickButton and tCursor.selectedItem == nIndex + nItems) or
                        not cosuConf.bDoubleClickButton then
                        tB.func(tPopup[1], tB)
                    end
        ...
        if type(tPopup[1].textBox) == "table" then
            for nIndex, tTB in pairs(tPopup[1].textBox) do
                if nX >= tPopup[1].x + tTB.x and nX < tPopup[1].x + tTB.x + tPopup[1].size.x and nY == tPopup[1].y +
                    tTB.y then
...
function input.toolbar.mouseClick(nButton, nX, nY)
    if nY == 1 then
        ...
        if nCategory > 0 then
            local nItems, nLength = category.items(nCategory, nil, true)
...
function input.handle.insert(event)
    ...
    if event[1] == "key" then
        if event[2] == keys.f5 and not event[4] then
        ...
        if arg then
            ...
            if fs.exists(sPath) then
                if fs.isDir(sPath) then
                    error("Cannot consult a directory.")
                    return false
                end
